 ...
 * @to do 解耦
 */
define(['jquery','responser'],function($, responser){
	...
	var onResponse = function(json){
		...
		var data = json,
				status = data.status,
				text = data.statustext || data.statusText

		if(text == 'Client not login.'){
			S3.CONFIG.isLogin = false
			//阻止登录前的所有请求
			block = true
			$.each(uniques, function(o){
				if(o.current){
					o.current.abort()
				}
			})
			//登录初始化后，试图提交时，恢复请求
			// $(document).on('requestLogin', function(){block = false})
			// S3.passport.check()
			window.location.href="/user/login/page"
			return
		}else if(text == 'Account needs activate.'){
			block == true
			$.each(uniques, function(o){
				if(o.current){
					o.current.abort()
			...
			window.location.href= '/bind3rd/open/service'
			return
		}
		if( status == 1 && text == "success" ){
			S3.CONFIG.isLogin = true
			return
		}
		responser.show({
			title: '错误',
